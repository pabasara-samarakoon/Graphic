...
void boundaryFill4(int x, int y,struct Color fill_colour, struct Color border_colour) 
{ 
	struct 	Color color;
	color=getPixelColor(x,y);

	if(color.r!=border_colour.r && color.g!=border_colour.g && color.b!=border_colour.b && color.r!=fill_colour.r && color.g!=fill_colour.g && color.b!=fill_colour.b)
	{
		setPixelColor(x, y, fill_colour);
		boundaryFill4(x+1, y, fill_colour, border_colour);
		boundaryFill4(x, y+1, fill_colour, border_colour);
		boundaryFill4(x-1, y, fill_colour, border_colour);
		boundaryFill4(x, y-1, fill_colour, border_colour);
	}
} 
...
void keyboard(unsigned char key, int x, int y)
{
	...
	else if (key == 'f' || key == 'F')
	{
		glClear(GL_COLOR_BUFFER_BIT);
		bresenhamCircle (xi, yi, r);
		boundaryFill4(xi,yi, fillColour, boderColour);
		glFlush();
	}
}
